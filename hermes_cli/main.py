...
def _apply_profile_override() -> None:
    ...
    # when it points at a concrete profile. If it points at the Hermes root,
    # still honor active_profile below (systemd/container setups may export the
    # root while the user's sticky active profile should apply).
    existing_home = os.environ.get("HERMES_HOME")
    if profile_name is None and existing_home:
        try:
            from hermes_constants import get_default_hermes_root

            if Path(existing_home).expanduser().resolve() != get_default_hermes_root().expanduser().resolve():
                return
        except Exception:
            return
...
def _run_post_update_steps(
    ...
    # breaks on this machine, keep base deps and reinstall the remaining extras
    # individually so update does not silently strip working capabilities.
    # Drop the interrupted-install breadcrumb BEFORE touching the venv. If the
    # install is killed mid-flight, the next ``hermes`` launch can finish the
    # install via ``_recover_from_interrupted_install``.
    ...
    # breadcrumb now — the remaining steps are non-core and can't brick the venv.
    _clear_update_incomplete_marker()

    _refresh_active_lazy_features()

    _update_node_dependencies()
    # Core is now complete; the dashboard build is optional from a CLI
    # perspective, and _build_web_ui() can fall back to stale dist assets.
    ...
    if (desktop_dir / "package.json").exists() and shutil.which("npm") and has_desktop_app:
        ...
        # Stream the build output live (long Electron builds otherwise look hung).
        # On the rare nonzero exit, retry once.
    ...
    # instead of waiting on a network fetch. Non-fatal on failure.
    try:
        from hermes_cli.model_catalog import seed_cache_from_checkout
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        if is_fork and current_branch != "main" and branch == "main":
            ...
            # No upstream changes — check the current fork branch below.
