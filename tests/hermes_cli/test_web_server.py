...
class TestPtyWebSocket:
    ...
    def test_ws_non_loopback_client_allowed_only_for_wildcard_bind(self, monkeypatch):
        ...
        # Non-wildcard bind: external clients are rejected
        monkeypatch.setattr(
            self.ws_module.app.state, "bound_host", "127.0.0.1", raising=False
        )
        assert not self.ws_module._is_public_bind()
        fake_ws = MagicMock()
        fake_ws.client.host = "100.64.131.115"
        assert not self.ws_module._ws_client_is_allowed(fake_ws)

        # Wildcard bind: external clients allowed
        monkeypatch.setattr(
            self.ws_module.app.state, "bound_host", "0.0.0.0", raising=False
        )
        assert self.ws_module._is_public_bind()
        fake_ws2 = MagicMock()
        fake_ws2.client.host = "100.64.131.115"
        assert self.ws_module._ws_client_is_allowed(fake_ws2)
